...
> **Note**: By default, Strands Agents use a `PrintingCallbackHandler` that streams responses to stdout as they’re generated. When you call `agent("What is 2+2")`, you’ll see the response appear in real-time as it’s being generated. The `print(response)` above also shows the final collected result after the response is complete. See [Callback Handlers](/docs/user-guide/concepts/streaming/callback-handlers/index.md) for more details.
...
> **Note**: The `web_search` and `email_sender` tools in this example are custom tools that you would need to define. See [Python Tools](/docs/user-guide/concepts/tools/custom-tools/index.md) for guidance on creating custom tools, or use existing tools from the [strands\_tools package](/docs/user-guide/concepts/tools/community-tools-package/index.md).
...
> **Note**: Structured output disables streaming and returns the complete response at once, unlike regular chat completions, which stream by default. See [Callback Handlers](/docs/user-guide/concepts/streaming/callback-handlers/index.md) for more details.
