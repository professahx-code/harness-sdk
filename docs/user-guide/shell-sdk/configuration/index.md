...
When the policy is static, declare it in a TOML file. The Python and Node.js constructors accept a config file path, and the [MCP server](/docs/user-guide/shell-sdk/mcp-server/index.md) reads the same format through its `--config` flag. Pass a file and explicit constructor arguments together, and the explicit arguments win.
...
-   [Commands](/docs/user-guide/shell-sdk/commands/index.md): the full command inventory and supported flags.
-   [MCP Server](/docs/user-guide/shell-sdk/mcp-server/index.md): serve this configuration to any MCP-compatible agent.
-   [Security Model](/docs/user-guide/shell-sdk/security/index.md): how binds, credential injection, and the SSRF guard hold up, and where they stop.